  ...
  .controller('OrganizationseditCtrl',['$scope', '$state', '$stateParams','OrganizationsService', function ($scope, $state, $stateParams, OrganizationsService) {
    ...
    $scope.autoCompleteOwner = function(cssClass, index){
      $( "."+cssClass ).autocomplete({
        ...
        select: function ( event, selectedItem) {
          $scope.newOwner.name[index] = selectedItem.item.value.name;
    ...
    $scope.updateOwner=function(){
      ...
      angular.forEach($scope.newOwner.name, function(item){
        ...
        $scope.organization.owners.push({"_id":newOwnerId});
        index++;
      })
    };
    ...
    $scope.updateProject=function(){
      ...
      angular.forEach($scope.newProject.name, function(item){
        ...
        $scope.organization.projects.push({"_id":newProjectId});
        index++;
      })
    };
    ...
    $scope.save = function(){
      ...
      $scope.updateProject();
      if($stateParams.id === undefined){
        OrganizationService.add($scope.organization, function(organization){
          $state.transitionTo("organizations.list");
        });
      }else{
        OrganizationsService.update($scope.organization, function(organization){
          $scope.organization = organization;
          $state.transitionTo('organizations.view',{"id" : $scope.organization._id});
        });
      }
    }
